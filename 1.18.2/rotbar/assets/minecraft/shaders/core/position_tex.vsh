...
bool check_bounds(vec2 uv, int vid, vec2 start, vec2 size)
{
	...
	bool l = !(vid == 0 || vid == 3) && uv.x == start.x;
	bool r = !(vid == 1 || vid == 2) && uv.x == start.x + size.x;
	bool u = !(vid == 2 || vid == 3) && uv.y == start.y;
	bool d = !(vid == 0 || vid == 1) && uv.y == start.y + size.y;
	return core && !(l || r || u || d);
}
...
void main() {
	...
	gl_Position.x += 39.0/ScreenSize.x * 2.0 * gui_scale * float(check_hotbar && (gl_VertexID == 1 || gl_VertexID == 2));
    ...
    texCoord0 = UV0;
    texCoord0.x += 39.0 / size.x * float(check_hotbar && (gl_VertexID == 1 || gl_VertexID == 2));
