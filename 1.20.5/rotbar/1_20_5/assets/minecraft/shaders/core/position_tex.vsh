...
out float[6] timer_digits;
out float[5] fog_digits;
...
void main() {
	...
	int timer_magnitude = int(24000.0 * abs(GameTime));
	float pitch = asin(clamp(IViewRotMat[2][1], -1.0, 1.0)) * 180.0 / 3.14159265358979;
	int pitch_magnitude = int(ModelViewMat[0][3] * 100.0);//int(100.0 * abs(pitch));
	int fog_magnitude = int(10.0 * abs(FogEnd));

	timer_digits[0] = float(sign(GameTime) < -0.5) - 2.0;// sign of timer; -1 for negative, -2 for positive
	timer_digits[1] = float((timer_magnitude % 100000) / 10000);
	timer_digits[2] = float((timer_magnitude % 10000) / 1000);
	timer_digits[3] = float((timer_magnitude % 1000) / 100);
	timer_digits[4] = float((timer_magnitude % 100) / 10);
	timer_digits[5] = float((timer_magnitude % 10));
	timer_digits[1] -= 2.0 * float(timer_digits[1] == 0.0 && timer_digits[0] == -2.0) + 1.0 * float(timer_digits[1] == 0.0 && timer_digits[0] == -1.0); // -2 for don't draw anything
	timer_digits[2] -= 2.0 * float(timer_digits[2] == 0.0 && timer_digits[1] == -2.0) + 1.0 * float(timer_digits[2] == 0.0 && timer_digits[1] == -1.0); // -2 for don't draw anything
	timer_digits[3] -= 2.0 * float(timer_digits[3] == 0.0 && timer_digits[2] == -2.0) + 1.0 * float(timer_digits[3] == 0.0 && timer_digits[2] == -1.0); // -2 for don't draw anything
	timer_digits[4] -= 2.0 * float(timer_digits[4] == 0.0 && timer_digits[3] == -2.0) + 1.0 * float(timer_digits[4] == 0.0 && timer_digits[3] == -1.0); // -2 for don't draw anything
	timer_digits[0] -= 1.0 * float(timer_digits[0] == -1.0 && timer_digits[1] == -1.0);
	timer_digits[1] -= 1.0 * float(timer_digits[1] == -1.0 && timer_digits[2] == -1.0);
	timer_digits[2] -= 1.0 * float(timer_digits[2] == -1.0 && timer_digits[3] == -1.0);
	timer_digits[3] -= 1.0 * float(timer_digits[3] == -1.0 && timer_digits[4] == -1.0);

	fog_digits[0] = float(sign(FogEnd) < -0.5) - 2.0; // sign of fog; -1 for negative, -2 for positive
	fog_digits[1] = float((fog_magnitude % 10000) / 1000);
	fog_digits[2] = float((fog_magnitude % 1000) / 100);
	fog_digits[3] = float((fog_magnitude % 100) / 10);
	fog_digits[4] = float((fog_magnitude % 10) / 1);
	...
	float prelim_yaw_direction = float(int(0.01 * float(timer_magnitude + 2250) / 45.0)) * (float(sign(IViewRotMat[0][2]) < 0.5) * 2.0 - 1.0);
