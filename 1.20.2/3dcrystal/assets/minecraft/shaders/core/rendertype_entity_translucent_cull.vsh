...
void main() {
	...
	float check_hand = float(check_ui && ProjMat[1].g > 0.0); // is the crystal in the player's hand in firstperson?
	float check_inventory = float(check_ui) * (1.0 - check_hand); // is the crystal in a GUI?
	float check_inventory_hand = check_inventory * float(Position.z < 100.0); // is this crystal in the player's hand in the GUI?
	...
	float model_scale = 0.125 // base scale
					  + 0.125 * check_hand // larger if in firstperson hand
					  + 4.875 * check_inventory // even larger if in inventory
					  - 0.75 * check_inventory_hand; // slightly smaller if in inventory and held by the character model since inventory scales it too big
	...
	model_scale *= -1.0 + 2.0 * check_inventory; // flip crystal back to normal if its not in a ui hand since for some reason they start inside-out
